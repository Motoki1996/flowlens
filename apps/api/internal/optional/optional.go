// Package optional holds the three-state field type PATCH request bodies are
// built from. It lives in its own package rather than in internal/task because
// internal/task imports internal/backlog (task.NewService takes the backlog
// service), so backlog cannot import task back for the same type.
package optional
...
// an UpdateParams carries Optional fields, and Update leaves every absent one
// at its current value instead of overwriting it with a zero.
//
