...
func (s *Service) Compute(ctx context.Context, ownerID, projectID uuid.UUID, from, to *time.Time) (Metrics, error) {
	...
	for _, row := range rows {
		...
		if row.GitlabCreatedAt.Valid && row.FirstReviewedAt.Valid {
			openToFirstReview = append(openToFirstReview, row.FirstReviewedAt.Time.Sub(row.GitlabCreatedAt.Time).Hours())
		}
		if row.FirstReviewedAt.Valid && row.MergedAt.Valid {
			firstReviewToMerge = append(firstReviewToMerge, row.MergedAt.Time.Sub(row.FirstReviewedAt.Time).Hours())
		}
	}

	var pipelineSuccessRate *float64
	if decided := pipelineSuccess + pipelineFailed; decided > 0 {
		rate := float64(pipelineSuccess) / float64(decided)
		pipelineSuccessRate = &rate
	}
